    ...
    protected void channelRead0(ChannelHandlerContext ctx, FullHttpRequest msg) throws Exception {
        try {
            ...
            for (String url : this.destinations) {
                try {     
                    ByteBuf copiedContent = msg.content().copy();
                    FullHttpRequest msgCopy = new DefaultFullHttpRequest(
                        msg.protocolVersion(),
                        msg.method(),
                        msg.uri(),
                        copiedContent
                    ...
                    msgCopy.headers().set(msg.headers());
                    msgCopy.trailingHeaders().set(msg.trailingHeaders());
                    
                    // Create ForwardHttp1 directly to avoid ByteBuf reference issues
                    String method = msgCopy.method().name();
                    String uri = msgCopy.uri();
                    Map<String, String> headers = new HashMap<>();
                    for (Map.Entry<String, String> entry : msgCopy.headers()) {
                        headers.put(entry.getKey().toLowerCase(), entry.getValue());
                    }
                    String clientAddress = ctx.channel().remoteAddress().toString();
                    
                    ForwardHttp1 clusterRequest = new ForwardHttp1(copiedContent, method, uri, headers, clientAddress);
                    ...
                    if (url.contains("://")) {
                        // Full URL format
                        host = HttpUtil.getHostFromURI(url);
                        port = HttpUtil.getPortFromURI(url);
                        path = HttpUtil.getPathFromURI(url);
                    } else {
